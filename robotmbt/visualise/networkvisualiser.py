...
OUTER_WINDOW_WIDTH: int = INNER_WINDOW_WIDTH + \
    (280 if LEGEND_PLACE == 'left' or LEGEND_PLACE == 'right' else 30)
OUTER_WINDOW_HEIGHT: int = INNER_WINDOW_HEIGHT + \
    (150 if LEGEND_PLACE == 'below' or LEGEND_PLACE == 'center' else 30)
...
class NetworkVisualiser:
    ...
    def _add_edges(self, nodes: list[Node], edges: list[Edge]):
        ...
        edge_part_glyph = Segment(
            x0='start_x', y0='start_y', x1='end_x', y1='end_y', line_color='color')
    ...
    def _add_features(self, suite_name: str, seed: str):
        ...
        if seed != "":
            self.plot.add_layout(
                Title(text="seed=" + seed, align="center", text_color="#999999"), "above")
        ...
        self.plot.y_range = Range1d(-INNER_WINDOW_HEIGHT +
                                    (0 if seed == '' else 20), 0)
        self.plot.x_range.tags = [{"initial_span": INNER_WINDOW_WIDTH}]
        self.plot.y_range.tags = [
            {"initial_span": INNER_WINDOW_HEIGHT - (0 if seed == '' else 20)}]
    ...
    def _add_legend(self, graph: AbstractGraph):
        ...
        final_trace_node_glyph = Rect(
            x='_', y='_', width='_', height='_', fill_color=FINAL_TRACE_NODE_COLOR)
        final_trace_node = self.plot.add_glyph(
            empty_source, final_trace_node_glyph)

        other_node_glyph = Rect(x='_', y='_', width='_',
                                height='_', fill_color=OTHER_NODE_COLOR)
        ...
        final_trace_edge = self.plot.add_glyph(
            empty_source, final_trace_edge_glyph)
        ...
        legend = Legend(items=[(graph.get_legend_info_final_trace_node(), [final_trace_node]),
                               (graph.get_legend_info_other_node(),
                                [other_node]),
                               (graph.get_legend_info_final_trace_edge(),
                                [final_trace_edge]),
...
def _add_edge_to_sources(nodes: list[Node], edge: Edge, final_trace: list[str], edge_part_source: ColumnDataSource,
    ...
    if edge.from_node == edge.to_node:
        _add_self_loop_to_sources(
            nodes, edge, in_final_trace, edge_arrow_source, edge_bezier_source, edge_label_source)
        return
    ...
    for i in range(len(edge.points) - 1):
        ...
        if i < len(edge.points) - 2:
            ...
            edge_part_source.data['color'].append(
                FINAL_TRACE_EDGE_COLOR if in_final_trace else OTHER_EDGE_COLOR)
        else:
            ...
            edge_arrow_source.data['color'].append(
                FINAL_TRACE_EDGE_COLOR if in_final_trace else OTHER_EDGE_COLOR)
...
def _add_self_loop_to_sources(nodes: list[Node], edge: Edge, in_final_trace: bool, edge_arrow_source: ColumnDataSource,
    ...
    edge_bezier_source.data['color'].append(
        FINAL_TRACE_EDGE_COLOR if in_final_trace else OTHER_EDGE_COLOR)
